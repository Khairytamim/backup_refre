...
class Transaksi_model extends CI_Model {
        ...
        public function tambah(){
                ...
                $kodepos = $this->input->post('kode_pos_transaksi');
                $kota = $this->input->post('kota_transaksi');
                $hasil=$this->db->query("call tambah_transaksi_trial('$nama', '$alamat', '$kontak','$harga','$produk','$email','$kodepos','$kota')");
                //$hasil=$this->db->query("call vendor_transaksi_trial('$produk')");
                $this->inputto_email($email,$nama,$produk,$alamat,$kontak,$harga);
                foreach ($hasil->result() as $col)
		{
			$namatoko = $col->nama_toko;
			$emailtoko = $col->email_toko;
		}
               	$this->inputto_finance($namatoko,$emailtoko,$alamat,$kontak,$produk,$nama,$kodepos,$kota);
                

        }

	public function inputto_email($email,$auth,$produk,$alamat,$kontak,$harga)
	{
		$link=base_url();
        ...
        public function inputto_finance($namatoko,$emailtoko,$alamat,$kontak,$produk,$nama,$kodepos,$kota)
	{
		$link=base_url();
		// Set SMTP Configuration
		$emailConfig = array(
		...
		<p>Berikut rincian pemesanan oleh $nama:</p>
			<img src='$link/assets/gambar_kaos/small/1/$produk.jpg'>
			<p>Nomor HP: $kontak</p>
			<p>Alamat dan Kode Pos: $alamat, $kota, $kodepos</p><br />
...
/*	
	public function inputto_vendor($auth,$email,$alamat,$kontak,$produk,$nama)
	{
		$link=base_url();
		// Set SMTP Configuration
		$emailConfig = array(
...
public function get_ukuran($id){
	...
	public function get_produk($id){
        ...
        public function select_detail_ukuran_barang($id, $tipe){
		$this->db->reconnect();
		$query = $this->db->query("call select_detail_ukuran_barang('$id', '$tipe')");
		return $query;
	}
        ...
        public function get_id_billing(){
        	$this->db->reconnect();
	        $hasil=$this->db->query("call get_id_terakhir_billing()");
	        return $hasil;
        }
        ...
        public function get_user(){
        	...
        	$this->db->reconnect();
	        $hasil=$this->db->query("select id_user, nama_user, email_user, nomor_telepon_user, kode_pos, kota, 
	        	provinsi, alamat_user, id_kota from user where id_user = '$id_user'");
	        return $hasil;
        }
}